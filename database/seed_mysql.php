...
/**
 * Инициализация БД fitlife в MySQL OSPanel с корректной UTF-8 кодировкой.
 * Запуск: php database/seed_mysql.php
 */
...
$pdo->exec('DROP DATABASE IF EXISTS fitlife');
$pdo->exec('CREATE DATABASE fitlife CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');
$pdo->exec('USE fitlife');
...
foreach ($users as $u) {
    $stmtUser->execute([$u[0], $u[1], $u[2], $u[3], '']);
}
...
$stmtArticle = $pdo->prepare('INSERT INTO articles (author_id, name, text) VALUES (?, ?, ?)');
foreach ($articles as $a) {
    $stmtArticle->execute($a);
}
...
$stmtComment = $pdo->prepare('INSERT INTO comments (author_id, article_id, text) VALUES (?, ?, ?)');
foreach ($comments as $c) {
    $stmtComment->execute($c);
}

echo "База fitlife пересоздана с UTF-8.\n";
